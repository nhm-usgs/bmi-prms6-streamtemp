    ...
    type :: prms_streamtemp_model
        ...
        real(r64) :: dummy_r64
        real(r32) :: dummy_r32
    end type prms_streamtemp_model
    ...
    type(prms_streamtemp_model), target, intent(out) :: model
    real(r64) :: dummy_r64
    real(r32) :: dummy_r32
    ...
    use, intrinsic :: iso_fortran_env, only: output_unit
    implicit none
    type(prms_streamtemp_model), intent(inout) :: model
    associate(start_rtc => model%start_rtc, end_rtc => model%end_rtc, max_rtc => model%max_rtc, &
        rate_rtc => model%rate_rtc, delta_rtc_sec => model%delta_rtc_sec,  &
        start_ct => model%start_ct, end_ct => model%end_ct, &
        model_simulation => model%model_simulation, &
